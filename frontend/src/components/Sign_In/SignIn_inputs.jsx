...
function SignIn_inputs() {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (isBarber) {
                userData.workStartTime = workStart;
                userData.workEndTime = workEnd;
            }
    ...
    return (
        <div className={Styles.SignIn_inputs_container}>
            <h3 className={Styles.title_register}>
                Cadastro de {isBarber ? 'Barbeiro' : 'Cliente'}
            </h3>
            ...
            <form>
                {step === 1 && (
                    <>
                        <label className={Styles.label_name}>
                            <p>Nome Completo:</p>
                            <input id={Styles.name_input} type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Digite seu nome completo" required />
                        </label>
                        <label className={Styles.label_email}>
                            <p>E-mail:</p>
                            <input id={Styles.email_input} type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seuemail@exemplo.com" required />
                        </label>
                        <label className={Styles.label_name}>
                            <p>CPF:</p>
                            <input id={Styles.email_input} type="text" value={cpf} onChange={(e) => setCpf(e.target.value)} placeholder="Somente numeros" required />
                        </label>
                        {error && <p style={{ color: '#ff4444', fontSize: '14px', marginTop: '8px' }}>{error}</p>}
                        <button className={Styles.SignIn_button} onClick={handleNextStep} type="button">Proximo</button>
                    </>
                )}
                ...
                {step === 2 && (
                    <>
                        <label className={Styles.label_name}>
                            <p>Telefone:</p>
                            <input id={Styles.name_input} type="text" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="11999999999" required />
                        </label>
                        {isBarber && (
                            <>
                                <label className={Styles.label_name}>
                                    <p>Inicio de Expediente:</p>
                                    <input id={Styles.name_input} type="time" value={workStart} onChange={(e) => setWorkStart(e.target.value)} required />
                                </label>
                                <label className={Styles.label_name}>
                                    <p>Fim de Expediente:</p>
                                    <input id={Styles.name_input} type="time" value={workEnd} onChange={(e) => setWorkEnd(e.target.value)} required />
                                </label>
                            </>
                        )}
                        <label className={Styles.label_name}>
                            <p>Senha:</p>
                            <input id={Styles.name_input} type="password" value={password} onChange={(e) => setPassword(e.target.value)} required />
                        </label>
                        <label className={Styles.label_name}>
                            <p>Confirmar senha:</p>
                            <input id={Styles.name_input} type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
                        </label>
                        {error && <p style={{ color: '#ff4444', fontSize: '14px', marginTop: '8px' }}>{error}</p>}
                        <button className={Styles.SignIn_button} onClick={handleRegister} disabled={loading} type="button">
            ...
            <p className={Styles.login_link} onClick={() => navigate('/login')} style={{ cursor: 'pointer' }}>Ja possui conta? Entrar</p>
        </div>
    );
}
