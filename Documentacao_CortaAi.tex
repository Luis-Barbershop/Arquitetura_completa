...
\begin{itemize}
    \item \textbf{Autenticação via Firebase:} O sistema utiliza o Firebase Authentication como provedor de identidade, suportando login por e-mail/senha e login social (Google). O API Gateway é o único ponto que valida o Firebase ID Token, injetando os headers \texttt{X-User-Id}, \texttt{X-User-Email} e \texttt{X-User-Role} para os microsserviços downstream.
    \item \textbf{Senhas locais com BCrypt:} Para usuários que optam pelo provedor \texttt{EMAIL}, a senha é armazenada com hash \texttt{BCryptPasswordEncoder}. Usuários de login social não possuem senha local (\texttt{password = null}).
    \item \textbf{Autorização Baseada em Papéis:} A arquitetura define \textit{roles} específicas (\texttt{ROLE\_CUSTOMER}, \texttt{ROLE\_BARBER}) injetadas pelo API Gateway após validação do token, implementando o princípio do menor privilégio.
    \item \textbf{Criptografia de Dados em Repouso (LGPD):} Campos sensíveis (e-mail, CPF, telefone, data de nascimento, CNPJ, tokens Mercado Pago) são criptografados em repouso com AES/GCM via \texttt{SensitiveStringConverter}. Um campo \texttt{email\_hash} (SHA-256) permite busca sem descriptografar.
    ...
    \item \textbf{Segurança na Infraestrutura:} Os microsserviços são acessíveis apenas internamente via Docker network. Somente o \texttt{api-gateway} está exposto publicamente. Credenciais são gerenciadas como variáveis de ambiente.
    \item \textbf{Direitos do Usuário:} O sistema garante aos usuários direitos de acesso, correção e exclusão de seus dados pessoais, com estratégia de anonimização (não deleção física) para preservar histórico financeiro.
\end{itemize}
