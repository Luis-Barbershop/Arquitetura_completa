...
- **Arquitetura:** **100% microsserviços.** Não existe mais monolito — a migração foi concluída. Qualquer referência a "monolito legado" em documentos ou comentários é histórica e deve ser ignorada.
- **Branch atual:** `feature/migracao-microservicos` (branch de trabalho ativa).
...
- Prefira a solução que **respeita o isolamento entre microsserviços** (banco próprio, contrato via DTO, comunicação via Feign/RabbitMQ).
