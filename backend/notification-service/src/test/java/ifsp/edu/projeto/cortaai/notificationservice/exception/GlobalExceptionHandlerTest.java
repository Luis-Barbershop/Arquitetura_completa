...
class GlobalExceptionHandlerTest {
    ...
    void setUp() {
        ...
        ReflectionTestUtils.setField(handler, "serviceName", "notification-service");
        request = new MockHttpServletRequest("GET", "/api/notifications");
        request.addHeader("X-Correlation-Id", "cid-notification");
    }

    @Test
    void shouldMapNotFoundVariants() throws Exception {
        assertError(handler.handleEntityNotFound(new EntityNotFoundException("notificação não encontrada"), request),
                HttpStatus.NOT_FOUND, "notificação não encontrada", "EntityNotFoundException");
    ...
    void shouldMapConflictAndUnsupportedMediaType() {
        assertError(handler.handleIllegalState(new IllegalStateException("notificação já lida"), request),
                HttpStatus.CONFLICT, "notificação já lida", "IllegalStateException");
    ...
    void shouldGenerateCorrelationIdWhenHeaderIsMissing() {
        MockHttpServletRequest requestWithoutHeader = new MockHttpServletRequest("GET", "/api/notifications/1");
        ...
        assertThat(response.getBody().getPath()).isEqualTo("/api/notifications/1");
    }
    ...
    private void assertError(ResponseEntity<ApiErrorResponse> response,
        ...
        assertThat(response.getBody().getOrigin()).isEqualTo("notification-service");
        assertThat(response.getBody().getPath()).isEqualTo("/api/notifications");
        assertThat(response.getBody().getCorrelationId()).isEqualTo("cid-notification");
    }
}
